...
namespace API_Data.src.Repository
{
    public class ConsultaRepository : IConsultaRepository
    {
        ...
        public async Task<Decimal> ValorLancamento(int ano, int mes)
        {
            try
            {
                ...
                var TotalDespesaLancamento = await _db.LancamentoParcelas
               .AsNoTracking()
               .Where(p =>
                   p.Status != StatusParcela.Pago &&
                   p.Lancamento.Categoria.Atribuicao == Atribuicao.Despesa &&
                   p.DataVencimento < fimMes
               )
               .SumAsync(p => p.ValorParcela);

                return TotalDespesaLancamento;
            }
            catch
            {
                return 0;
        ...
        public async Task<Decimal> ValorContaFixa(int ano, int mes)
        {
            try
            {
                // 1. Define o primeiro dia do mês solicitado (ex: 01/07/2026)
                ...
                var TotalDespesaContaFixa = await _db.ContaFixaParcelas
               .AsNoTracking()
               .Where(p =>
                   p.Status != StatusParcela.Pago &&
                   p.ContaFixa.Categoria.Atribuicao == Atribuicao.Despesa &&
                   p.DataVencimento < fimMes
               )
               .SumAsync(p => p.ValorParcela);

                return TotalDespesaContaFixa;
            }
            catch
            {
                return 0;
        ...
        /// </summary>
        public async Task<Decimal> TotalPagasDoMes(int ano, int mes)
        {
            try
            {
        ...
        public async Task<Decimal> TotalContasApgar(int ano, int mes)
        {
            try
            {
        ...
        /// Que estejam com vencimento até o mês/ano informado.
        /// </summary>
        public async Task<decimal> TotalContasReceber(int ano, int mes)
        {
            try
            {
        ...
        /// </summary>
        public async Task<Decimal> TotalContasSaldo(int ano, int mes)
        {
            try
            {
